...
def _running_in_wsl() -> bool:
    """Return True when running WSL"""
    return sys.platform == "linux" and ("WSL_INTEROP" in os.environ or "WSL_DISTRO_NAME" in os.environ)

if (sys.platform == "win32" or _running_in_wsl()) and "TEXTUAL_COLOR_SYSTEM" not in os.environ:
...
class TodoApp(App):
                ...
                with open(data_path, "r", encoding="utf-8") as f:
                    ...
                    if "tabs" in data:
                        ...
                        for tab_id, tab_name in tab_names.items():
                            tab_tasks = tasks_by_tab.get(
                                tab_id, {"not_completed": [], "completed": []}
                            )
                            migrated_tabs.append(
                                {
                                    ...
                                    "tasks": {
                                        "not_completed": list(
                                            tab_tasks.get("not_completed", [])
                                        ),
                                        "completed": list(
                                            tab_tasks.get("completed", [])
                                        ),
                                    },
                                }
                            )
    ...
    def _save_data_after_refresh(self) -> None:
        ...
        for tab in tabs_widget.query(Tab):
            ...
            tab_tasks = self.tasks_by_tab.get(
                tab_id, {"not_completed": [], "completed": []}
            )
            tabs_data.append(
                {
        ...
        if self.saved_tabs:
            ...
            if tabs.active_tab:
                ...
                if self.current_tab_id not in self.tasks_by_tab:
                    self.tasks_by_tab[self.current_tab_id] = {
                        "not_completed": [],
                        "completed": [],
                    }
    ...
    def action_close_modal(self) -> None:
        ...
        if tab_modal.visible:
            ...
            not_completed = list(
                self.query_one("#not_completed_tasks").query(TaskRadioButton)
            )
            if not_completed:
    ...
    def add_todo_item(self) -> None:
        ...
        if todo_text and self.current_tab_id is not None:
            ...
            if self.current_tab_id in self.tasks_by_tab:
                self.tasks_by_tab[self.current_tab_id]["not_completed"].append(
                    str(todo_text)
                )
            self._save_data()
    ...
    def on_radio_button_changed(self, event: RadioButton.Changed) -> None:
        ...
        try:
            if (
                ...
                and self.current_tab_id in self.tasks_by_tab
            ):
                if task_widget.value:
                    self.query_one("#completed_tasks").mount(
                        TaskRadioButton(label=task_text, value=True, compact=True)
                    )
                    task_widget.remove()
                    task_str = str(task_text)
                    if (
                        task_str
                        in self.tasks_by_tab[self.current_tab_id]["not_completed"]
                    ):
                        self.tasks_by_tab[self.current_tab_id]["not_completed"].remove(
                            task_str
                        )
                        self.tasks_by_tab[self.current_tab_id]["completed"].append(
                            task_str
                        )
                else:
                    self.query_one("#not_completed_tasks").mount(
                        TaskRadioButton(
                            label=task_text, value=False, compact=self.compact
                        )
                    )
                    task_widget.remove()
                    task_str = str(task_text)
                    if task_str in self.tasks_by_tab[self.current_tab_id]["completed"]:
                        self.tasks_by_tab[self.current_tab_id]["completed"].remove(
                            task_str
                        )
                        self.tasks_by_tab[self.current_tab_id]["not_completed"].append(
                            task_str
